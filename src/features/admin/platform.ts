...
type BadgeQuery = {
  select(columns: string): BadgeQuery;
  order(
    column: string,
    options?: { ascending?: boolean },
  ): Promise<BadgeQueryResult>;
};

type BadgeClient = {
  from(table: "tibo_badges"): BadgeQuery;
};

const badgeDb = db as unknown as BadgeClient;
...
export async function upsertPlatformTeam(userId: string, role: string) {
  const { error } = await db.rpc("admin_upsert_platform_team", {
    _user_id: userId,
    _role: role,
  });
...
export async function removePlatformTeam(userId: string) {
  const { error } = await db.rpc("admin_remove_platform_team", {
    _user_id: userId,
  });
...
export async function listBadges(): Promise<AdminBadge[]> {
  ...
  // A tabela ainda não está presente na tipagem gerada do Supabase.
  const badgesTable = db as unknown as {
    from: (table: string) => {
  ...
  const { data, error } = await badgesTable
    .from("tibo_badges")
    .select("*")
    .order("sort_order")
...
export async function createBadge(input: {
  ...
  const { data, error } = await db.rpc("admin_create_badge", {
    _name: input.name,
    _description: input.description ?? "",
    _image_path: input.imagePath,
    _category: input.category ?? "",
    _level: input.level ?? 1,
  });
...
export async function grantBadge(userId: string, badgeId: string) {
  const { error } = await db.rpc("admin_grant_badge", {
    _user_id: userId,
    _badge_id: badgeId,
  });
...
export async function revokeBadge(userId: string, badgeId: string) {
  const { error } = await db.rpc("admin_revoke_badge", {
    _user_id: userId,
    _badge_id: badgeId,
  });
...
export async function setMyBadgeVisibility(
  id: string,
  visible: boolean,
  order = 0,
) {
  const { error } = await db.rpc("set_my_badge_visibility", {
    _user_badge_id: id,
    _is_visible: visible,
